...
func TestNegotiateVersion_CompatibleMajor(t *testing.T) {
	...
	srv := &Server{handler: staticMCPHandler{serverName: "engram", serverVersion: "v4.5.0"}}
	resp, err := srv.NegotiateVersion(context.Background(), &pb.NegotiateVersionRequest{ClientVersion: "4.1.2"})
	require.NoError(t, err)
	assert.True(t, resp.Compatible)
	assert.Equal(t, "v4.5.0", resp.ServerVersion)
...
func TestNegotiateVersion_IncompatibleMajor(t *testing.T) {
	...
	srv := &Server{handler: staticMCPHandler{serverName: "engram", serverVersion: "v4.5.0"}}
	resp, err := srv.NegotiateVersion(context.Background(), &pb.NegotiateVersionRequest{ClientVersion: "v3.9.9"})
	require.NoError(t, err)
	assert.False(t, resp.Compatible)
	assert.Equal(t, "v4.5.0", resp.ServerVersion)
	assert.Contains(t, resp.IncompatReason, "client major version 3")
	assert.Contains(t, resp.IncompatReason, "server major version 4")
}
