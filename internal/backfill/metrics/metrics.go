...
// SnapshotMetrics holds a point-in-time copy of Metrics values (without the mutex).
...
// Snapshot returns a point-in-time copy of the current metrics state (thread-safe).
func (m *Metrics) Snapshot() SnapshotMetrics {
	...
	return SnapshotMetrics{
		...
		SkippedTiny:     m.SkippedTiny,
		Processed:       m.Processed,
		TotalChunks:     m.TotalChunks,
		TotalObs:        m.TotalObs,
		ValidObs:        m.ValidObs,
		UniqueObs:       m.UniqueObs,
		DedupSkipped:    m.DedupSkipped,
		NoObsResponses:  m.NoObsResponses,
		MalformedXML:    m.MalformedXML,
		LLMErrors:       m.LLMErrors,
		ValidationErrs:  m.ValidationErrs,
